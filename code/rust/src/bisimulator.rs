...
impl InternalNode2BlockMapper {
    ...
    // TODO Check: do we need the mapping, mayeb it can be siomplified to not have the facade in front of old_mapping
    ...
    pub fn total_blocks(&self) -> usize {
        let non_singleton = self
            ...
            .filter(|b| match b {
                Some(b) => {
                    !b.nodes.is_empty()
                    //TODO: It seems there is an invariant here that could be exploited. There must never be any empty block that is not None.
                }
                None => false,
            })
            .count();
    ...
    F2: FnMut(&BlockIndex) -> Result<()>,
{
    ...
    //TODO We should have the new mapping shadow the old one, and replace in the end.
    let old_mapping = prev_outcome.node_to_block.mapping.clone();
    let dirty_blocks = prev_outcome.dirty_blocks;
    ...
    for dirty_idx in dirty_blocks {
        ...
        for &v in block_ref.nodes.iter() {
            let sig: Vec<(EdgeType, i64)> = graph.nodes[v]
                .edges
                .iter()
                .map(|e| (e.label, old_mapping[e.target]))
                .unique()
    ...
    for refinded_block in refined_block_set {
        for target in refinded_block.nodes.iter() {
