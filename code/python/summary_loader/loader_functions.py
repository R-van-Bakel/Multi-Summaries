...
def get_sizes(experiment_directory:str, depth:int, typed_start=True) -> list[dict[str,Counter[int,int]]]:
    ...
    start = 0 if typed_start else 1  # The untyped start has a trivial universal block at level 0, so it will not be stored explicitly
    for i in range(start,depth+1):
        outcome_binary_file = experiment_directory + f"bisimulation/outcome_condensed-{i:04d}.bin"
        assert os.path.isfile(outcome_binary_file), "The outcome (binary) file should exist"

        if i > start:
            assert os.path.exists(experiment_directory), "The experiment directory string should refer to a valid (existing) directory"
            mapping_binary_file = experiment_directory + f"bisimulation/mapping-{i-1:04d}to{i:04d}.bin"
            assert os.path.isfile(mapping_binary_file), "The mapping (binary) file should exist"
            ...
            with open(mapping_binary_file, "rb") as f:
                while (merged_bytes := f.read(BYTES_PER_BLOCK)):
                    ...
                    split_count = int.from_bytes(f.read(BYTES_PER_BLOCK), "little", signed=False)
...
def get_statistics(experiment_directory:str, depth:int, typed_start=True) -> any:
    ...
    start = 0 if typed_start else 1  # The untyped start has a trivial universal block at level 0, so it will not be stored explicitly
    for i in range(start,depth+1):
        stats_json_file = experiment_directory + f"ad_hoc_results/statistics_condensed-{i:04d}.json"
        assert os.path.isfile(stats_json_file), "The statistics (json) file should exist"
...
def get_data_edge_statistics(experiment_directory:str, depth:int, typed_start=True) -> any:
    assert os.path.exists(experiment_directory), "The experiment directory string should refer to a valid (existing) directory"
    statistics = []
    start = 0 if typed_start else 1  # The untyped start has a trivial universal block at level 0, so it will not be stored explicitly
    for i in range(start,depth):
        stats_json_file = experiment_directory + f"ad_hoc_results/data_edges_statistics_condensed-{i+1:04d}to{i:04d}.json"
        assert os.path.isfile(stats_json_file), "The statistics (json) file should exist"
    ...
    fixed_point_stats_json_file = f"ad_hoc_results/data_edges_statistics_condensed-{depth:04d}to{depth:04d}.json"
...
def get_graph_statistics(experiment_directory:str) -> any:
    ...
    assert os.path.isfile(graph_stats_json_file), "The statistics (json) file should exist"
...
def get_summary_graph_statistics(experiment_directory:str) -> any:
    assert os.path.exists(experiment_directory), "The experiment directory string should refer to a valid (existing) directory"
    summary_graph_stats_json_file = experiment_directory + f"ad_hoc_results/summary_graph_stats.json"
    assert os.path.isfile(summary_graph_stats_json_file), "The statistics (json) file should exist"
